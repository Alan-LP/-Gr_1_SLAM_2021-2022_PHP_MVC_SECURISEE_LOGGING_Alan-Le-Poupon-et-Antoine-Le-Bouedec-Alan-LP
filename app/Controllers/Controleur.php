...
public function index() {
	...
	elseif (isset($_POST['login'])) {
        ...
        if ($Modele->verifNbConnexion($ip)>= 10) {
            // log critique permettant de dire que le nombre de connexion autorisé est dépassé, il peut s'agir d'une attaque par force brute
            log_message('critical', 'Le nombre de connexion autorisé est dépassé pour cette adresse ip, il peut s\'agir d\'une attaque par force brute');
            echo view('Connexion');
        }
...
public function modifierFraisForfait() {
    ...
    //Log d'info indiquant que le visiteur Prenom Nom a modifié ses frais forfaits
    log_message('info', 'Le visiteur ' . $_SESSION['prenom'] . ' ' . $_SESSION['nom'] . ' a modifié ses frais forfaits');
...
public function ajouterFraisHorsForfait() {
    ...
    //Log d'info indiquant que le visiteur Prenom Nom a ajouté un frais hors forfait
    log_message('info', 'Le visiteur ' . $_SESSION['prenom'] . ' ' . $_SESSION['nom'] . ' a ajouté un frais hors forfait');
...
public function supprimerFraisHorsForfait() {
    ...
    //Log d'info indiquant que le visiteur Prenom Nom a supprimé un frais hors forfait
    log_message('info', 'Le visiteur ' . $_SESSION['prenom'] . ' ' . $_SESSION['nom'] . ' a supprimé un frais hors forfait');
...
public function testconnexion($login, $mdp) {
        ...
        if (!empty($donnees[0]->id)) {
            ...
            // log d'info indiquant que le visiteur Prenom Nom s'est connecté
            log_message('info', 'Le visiteur ' . $_SESSION['prenom'] . ' ' . $_SESSION['nom'] . ' s\'est connecté');
        ...
        // enfin on affiche la page connexion
            ...
            // log d'erreur indiquant que l'utilisateur n'a pas réussi à se connecter
            log_message('error', 'L\'utilisateur n\'a pas réussi à se connecter');
            echo view("Connexion");
        }
}
